...
def convert_to_onnx(unet, img_encoder, quant_conv, post_quant_conv, decoder, text_encoder, height=512, width=512):
    """Convert given input models to onnx files.
        unet: UNet2DConditionModel
        img_encoder: AutoencoderKL.encoder
        quant_conv: AutoencoderKL.quant_conv
    ...
    # vae.encoder onnx export
    check_inputs = [(torch.rand(1, 3, h, w),), (torch.rand(2, 3, h, w),)]
    traced_model = torch.jit.trace(img_encoder, check_inputs[0], check_inputs=[check_inputs[1]])
    torch.onnx.export(traced_model, check_inputs[0], "onnx/img_encoder.onnx", 
        input_names=["init_image"], dynamic_axes={"init_image": [0]}, opset_version=16)

    # vae.quant_conv onnx export
    check_inputs = [(torch.rand(1, 8, h, w),), (torch.rand(2, 8, h, w),)]
    traced_model = torch.jit.trace(quant_conv, check_inputs[0], check_inputs=[check_inputs[1]])
    torch.onnx.export(traced_model, check_inputs[0], "onnx/quant_conv.onnx", 
        input_names=["init_image_latents"], dynamic_axes={"init_image_latents": [0]}, opset_version=16)

    # vae.post_quant_conv onnx export
    ...
    torch.onnx.export(traced_model, check_inputs[0], "onnx/text_encoder.onnx", input_names=["text_input"], 
        dynamic_axes={"text_input": [0, 1]}, opset_version=16)
    

# Change height and width to create ONNX model file for that size
convert_to_onnx(pipe.unet, pipe.vae.encoder,  pipe.vae.quant_conv,  pipe.vae.post_quant_conv, pipe.vae.decoder, text_encoder, height=512, width=512)
